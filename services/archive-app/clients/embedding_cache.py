...
DEFAULT_CACHE_PATH = Path(
    os.getenv("EMBEDDING_CACHE_PATH") or (_REPO_ROOT / "data" / "embeddings_cache.npz")
)
...
class EmbeddingCache:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            loaded_model = (
                bytes(data["model"]).decode("utf-8") if "model" in data else ""
            )
            ...
            for i, h in enumerate(hashes):
                h_str = (
                    bytes(h).decode("ascii")
                    if isinstance(h, (bytes, np.bytes_))
                    else str(h)
                )
