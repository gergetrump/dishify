...
def main() -> None:
	...
	parser.add_argument("--csv", type=Path, default=ROOT / "data" / "dataset_normalized_10000.csv")
	parser.add_argument("--limit", type=int, default=500, help="0 means load all rows")
	parser.add_argument("--no-embed", action="store_true", help="Skip embedding/vector index")
	...
	if qdrant_url:
		...
		print("QDRANT_URL not set; skipping Qdrant upload (in-memory store will be used).")
