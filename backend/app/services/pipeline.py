        ...
        with _stage(stages, "vector_retrieval") as st:
            try:
                ...
                if not candidates:
                    ...
                    ranked = fallback_rank_without_vectors(
                        pool, normalized, limit=top_k_retrieval
                    )
                else:
                    retrieved_recipes = get_by_ids(
                        db_session, [c.recipe_id for c in candidates]
                    )
