...
def _build_reasoning_index(results: list[ExplainResultItem]) -> dict[str, ReasoningDetail]:
...
def run_recommend_pipeline(request: RecommendRequest) -> RecommendResponse:
	...
	with httpx.Client(timeout=timeout) as client:
		...
		if retrieve_response.status_code == 503:
			raise PipelineUnavailableError(retrieve_response.json().get("detail", retrieve_response.text))
