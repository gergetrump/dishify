...
class KeycloakClient:
		...
		*,
		...
		with httpx.Client(timeout=self.timeout) as client:
			...
			if not user_id:
				...
				if not users:
					raise KeycloakError("User created but id not returned", status_code=500)
				user_id = users[0]["id"]
	...
	def find_users(self, *, username: str | None = None, email: str | None = None) -> list[dict[str, Any]]:
		params: dict[str, str] = {}
		if username:
...
def clear_preference_attributes(user_id: str, client: KeycloakClient) -> None:
	...
	if "exclusion_restrictions" not in attributes and "cuisine_preferences" not in attributes:
		return
