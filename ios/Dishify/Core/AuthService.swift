...
final class AuthService: NSObject, ObservableObject {
    ...
    private func loadAuthConfig() async throws -> AuthConfig {
        ...
        let normalizedConfig = normalizeAuthConfig(config)
        authConfig = normalizedConfig
    ...
    private func normalizeAuthConfig(_ config: AuthConfig) -> AuthConfig {
        AuthConfig(
            issuer: normalizeEndpoint(config.issuer),
            authorizationEndpoint: normalizeEndpoint(config.authorizationEndpoint),
            tokenEndpoint: normalizeEndpoint(config.tokenEndpoint),
            logoutEndpoint: normalizeEndpoint(config.logoutEndpoint),
            userinfoEndpoint: normalizeEndpoint(config.userinfoEndpoint),
            jwksURI: normalizeEndpoint(config.jwksURI),
            realm: config.realm,
    ...
    private func normalizeEndpoint(_ endpoint: String) -> String {
        ...
        // Some local backends expose Docker-internal OIDC hosts (e.g. "keycloak")
        // via /auth/config. Rewrite those to the app's configured Keycloak base URL.
        guard host == "keycloak" else {
            return endpoint
        }

        let baseURL = Config.keycloakBaseURL
